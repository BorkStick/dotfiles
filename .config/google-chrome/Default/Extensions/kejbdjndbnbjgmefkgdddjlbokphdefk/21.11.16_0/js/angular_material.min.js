...
$jscomp.POLYFILL_PREFIX+c),c=$jscomp.propertyToPolyfillSymbol[c],$jscomp.defineProperty(e,c,{configurable:!0,writable:!0,value:b})))};$jscomp.initSymbol=function(){};
...
null!=h;break a;case "function":l=!0;break a;default:l=!1}l?this.resolveToNonPromiseObj_(h):this.fulfill_(h)}};f.prototype.resolveToNonPromiseObj_=function(h){var l=void 0;try{l=h.then}catch(m){this.reject_(m);return}"function"==typeof l?this.settleSameAsThenable_(l,h):this.fulfill_(h)};f.prototype.reject_=function(h){this.settle_(e.REJECTED,h)};f.prototype.fulfill_=function(h){this.settle_(e.FULFILLED,h)};f.prototype.settle_=function(h,l){if(this.state_!=e.PENDING)throw Error("Cannot settle("+h+
...
function MdConstantFactory(){function a(l){var m=e+"-"+l;m=b(m);var v=m.charAt(0).toLowerCase()+m.substring(1);return angular.isDefined(c.style[l])?l:angular.isDefined(c.style[m])?m:angular.isDefined(c.style[v])?v:l}function b(l){return l.replace(g,function(m,v,n,q){return q?n.toUpperCase():n})}function d(l){var m,v=/^(Moz|webkit|ms)(?=[A-Z])/;for(m in l.style)if(l=v.exec(m))return l[0]}var c=document.createElement("div"),e=d(c),f=/webkit/i.test(e),g=/([:\-_]+(.))/g,h={isInputKey:function(l){return 31<=
l.keyCode&&90>=l.keyCode},isNumPadKey:function(l){return 3===l.location&&97<=l.keyCode&&105>=l.keyCode},isMetaKey:function(l){return 91<=l.keyCode&&93>=l.keyCode},isFnLockKey:function(l){return 112<=l.keyCode&&145>=l.keyCode},isNavigationKey:function(l){var m=h.KEY_CODE;m=[m.SPACE,m.ENTER,m.UP_ARROW,m.DOWN_ARROW];return-1!=m.indexOf(l.keyCode)},hasModifierKey:function(l){return l.ctrlKey||l.metaKey||l.altKey},ELEMENT_MAX_PIXELS:1533917,BEFORE_NG_ARIA:210,KEY_CODE:{COMMA:188,SEMICOLON:186,ENTER:13,
...
G||D.callback()});z&&c.$digest()}var A=y.nextTick,w=A.timeout,B=A.queue||[];B.push({scope:t,callback:k});null==p&&(p=!0);A.digest=A.digest||p;A.queue=B;return w||(A.timeout=b(H,0,!1))},processTemplate:function(k){return r?k:k&&angular.isString(k)?k.replace(/\{\{/g,n).replace(/}}/g,q):k},getParentWithPointerEvents:function(k){for(k=k.parent();x(k,"pointer-events","none");)k=k.parent();return k},getNearestContentElement:function(k){for(k=k.parent()[0];k&&k!==h[0]&&k!==document.body&&"MD-CONTENT"!==
...
function attachToDocument(a,b,d){function c(m){var v=!m.clientX&&!m.clientY;v||m.$material||m.isIonicTap||lastLabelClickPos&&lastLabelClickPos.x===m.x&&lastLabelClickPos.y===m.y||"mousedown"===m.type&&(canFocus(m.target)||canFocus(document.activeElement))||(m.preventDefault(),m.stopPropagation())}function e(m){var v;(v=d.isIos?angular.isDefined(m.webkitForce)&&0===m.webkitForce:0===m.clientX&&0===m.clientY)||m.$material||m.isIonicTap||lastLabelClickPos&&lastLabelClickPos.x===m.x&&lastLabelClickPos.y===
m.y?(lastLabelClickPos=null,"label"===m.target.tagName.toLowerCase()&&(lastLabelClickPos={x:m.x,y:m.y})):(m.preventDefault(),m.stopPropagation(),lastLabelClickPos=null)}function f(m,v){var n;for(n in HANDLERS){var q=HANDLERS[n];q instanceof b&&("start"===m&&q.cancel(),q[m](v,pointer))}}function g(m){if(!pointer){var v=+Date.now();lastPointer&&!typesMatch(m,lastPointer)&&1500>v-lastPointer.endTime||(pointer=makeStartPointer(m),f("start",m))}}function h(m){pointer&&typesMatch(m,pointer)&&(updatePointerState(m,
pointer),f("move",m))}function l(m){pointer&&typesMatch(m,pointer)&&(updatePointerState(m,pointer),pointer.endTime=+Date.now(),"pointercancel"!==m.type&&f("end",m),lastPointer=pointer,pointer=null)}disableAllGestures||(!isInitialized&&a.isHijackingClicks&&(document.addEventListener("click",e,!0),document.addEventListener("mouseup",c,!0),document.addEventListener("mousedown",c,!0),document.addEventListener("focus",c,!0),isInitialized=!0),angular.element(document).on("mousedown touchstart pointerdown",
...
function InterimElementProvider(){function a(d){function c(n){m.optionsFactory=n.options;m.methods=(n.methods||[]).concat(h);return v}function e(n,q){l[n]=q;return v}function f(n,q){q=q||{};q.methods=q.methods||[];q.options=q.options||function(){return{}};if(/^cancel|hide|show$/.test(n))throw Error("Preset '"+n+"' in "+d+" is reserved!");if(-1<q.methods.indexOf("_options"))throw Error("Method '_options' in "+d+" is reserved!");m.presets[n]={methods:q.methods.concat(h),optionsFactory:q.options,argOption:q.argOption};
...
var R=function(X){if("string"===typeof X)return Y.$setTheme(X);F.when(a.isFunction(X)?X():X).then(function(aa){Y.$setTheme(aa)})};R(Q());var U=S.$watch(Q,function(X){X&&(R(X),Y.$shouldWatch||U())})}}}}function e(){u.disableTheming=!0;return{restrict:"A",priority:"900"}}function f(D){return D}function g(D,G,N){m(D,G);N=N.replace(/THEME_NAME/g,D.name);var C=new RegExp("\\.md-"+D.name+"-theme","g");G=/'?"?\{\{\s*([a-zA-Z]+)-(A?\d+|hue-[0-3]|shadow|default)-?(\d\.?\d*)?(contrast)?\s*\}\}'?"?/g;N=N.replace(G,
function(S,K,M,I,J){if("foreground"===K)return"shadow"==M?D.foregroundShadow:D.foregroundPalette[M]||D.foregroundPalette["1"];if(0===M.indexOf("hue")||"default"===M)M=D.colors[K].hues[M];return n((r[D.colors[K].name][M]||"")[J?"contrast":"value"],I)});var F=/('|")?{{\s*([a-zA-Z]+)-(color|contrast)-?(\d\.?\d*)?\s*}}("|')?/g,O=[];a.forEach(["default","hue-1","hue-2","hue-3"],function(S){var K=N.replace(F,function(I,J,Q,T,Y){Q=D.colors[Q];I=r[Q.name];Q=Q.hues[S];return n(I[Q]["color"===T?"value":"contrast"],
...
this._state&&(b(this._doneCallbacks,function(r){r(q)}),this._doneCallbacks.length=0,this._state=2)}};n.all=function(q,r){function x(p){k=k&&p;++y===q.length&&r(k)}var y=0,k=!0;b(q,function(p){p.done(x)})};return n}];angular.module("material.core.animate",[]).factory("$$forceReflow",g).factory("$$AnimateRunner",l).factory("$$rAFMutex",h).factory("$animateCss",["$window","$$rAF","$$AnimateRunner","$$forceReflow","$$jqLite","$timeout","$animate",function(m,v,n,q,r,x,y){function k(C,F){var O=[],S=z(C),
K=S&&y.enabled(),M=!1,I=!1;K&&(F.transitionStyle&&O.push([c+"transition",F.transitionStyle]),F.keyframeStyle&&O.push([c+"animation",F.keyframeStyle]),F.delay&&O.push([c+"transition-delay",F.delay+"s"]),F.duration&&O.push([c+"transition-duration",F.duration+"s"]),M=F.keyframeStyle||F.to&&(0<F.duration||F.transitionStyle),I=!!F.addClass||!!F.removeClass,D(C,!0));var J=K&&(M||I);B(C,F);var Q=!1,T,Y;return{close:m.close,start:function(){function R(){if(!Q)return Q=!0,T&&Y&&C.off(T,Y),p(C,F),w(C,F),b(O,
function(X){S.style[a(X[0])]=""}),U.complete(!0),U}var U=new n;A(function(){D(C,!1);if(!J)return R();b(O,function(ca){var da=ca[0];ca=ca[1];S.style[a(da)]=ca});p(C,F);var X=t(C);if(0===X.duration)return R();var aa=[];F.easing&&(X.transitionDuration&&aa.push([c+"transition-timing-function",F.easing]),X.animationDuration&&aa.push([c+"animation-timing-function",F.easing]));F.delay&&X.animationDelay&&aa.push([c+"animation-delay",F.delay+"s"]);F.duration&&X.animationDuration&&aa.push([c+"animation-duration",
...
g.$attr.mdSvgSrc||"");g.role||(d.expect(f,"role","img"),g.role="img");if("img"===g.role&&!g.ariaHidden&&!d.hasAriaLabel(f)){var v;g.alt?d.expect(f,"aria-label",g.alt):d.parentHasAriaLabel(f,2)?d.expect(f,"aria-hidden","true"):(v=g.mdFontIcon||g.mdSvgIcon||f.text())?d.expect(f,"aria-label",v):d.expect(f,"aria-hidden","true")}e&&g.$observe(e,function(n){f.empty();n&&a(n).then(function(q){f.empty();f.append(q)})})}return{restrict:"E",link:c}}MdIconService.$inject="config $templateRequest $q $log $mdUtil $sce".split(" ");
...
m){function v(){var q=m.mdColors,r=-1<q.indexOf("::"),x=r?!0:d.test(m.mdColors);m.mdColors=q.replace("::","");q=angular.isDefined(m.mdColorsWatch);return r||x?!1:q?f.parseAttributeBoolean(m.mdColorsWatch):!0}var n=v();return function(q,r,x,y){var k=y[0],p={},t=function(w){"string"!==typeof w&&(w="");x.mdColors||(x.mdColors="{}");var B=h(x.mdColors)(q);k&&Object.keys(B).forEach(function(u){var z=B[u];e.hasTheme(z)||(B[u]=(w||k.$mdTheme)+"-"+z)});H(B);return B},H=function(w){if(!angular.equals(w,p)){var B=
...
r.mode||(n.addEventListener("click",this.monthCtrl.headerClickHandler),n.setAttribute("data-timestamp",e.getTime()),n.setAttribute("aria-label",this.dateLocale.monthFormatter(c)),n.classList.add("md-calendar-label-clickable"),n.appendChild(this.arrowIcon.cloneNode(!0)));if(2>=f){if(n.setAttribute("colspan","7"),c=this.buildDateRow(),c.appendChild(n),d.insertBefore(c,l),m)return d}else v=3,n.setAttribute("colspan","3"),l.appendChild(n);for(;v<f;v++)l.appendChild(this.buildDateCell());for(v=1;v<=g;v++){if(7===
f){if(m)return d;f=0;h++;l=this.buildDateRow(h);d.appendChild(l)}e.setDate(v);n=this.buildDateCell(e);l.appendChild(n);f++}for(;7>l.childNodes.length;)l.appendChild(this.buildDateCell());for(;6>d.childNodes.length;){g=this.buildDateRow();for(h=0;7>h;h++)g.appendChild(this.buildDateCell());d.appendChild(g)}return d};b.prototype.getLocaleDay_=function(d){return(d.getDay()+(7-this.dateLocale.firstDayOfWeek))%7}})();
...
u.getMonth(),b(u))}function x(u){return u&&u.getTime&&!isNaN(u.getTime())}function y(u){x(u)&&u.setHours(0,0,0,0)}function k(u){u=angular.isUndefined(u)?new Date:new Date(u);y(u);return u}function p(u,z,D){u=k(u);z=x(z)?k(z):null;D=x(D)?k(D):null;return(!z||z<=u)&&(!D||D>=u)}function t(u,z){return n(u,12*z)}function H(u,z){return z.getFullYear()-u.getFullYear()}function A(u,z,D){var G=u;z&&u<z&&(G=new Date(z.getTime()));D&&u>D&&(G=new Date(D.getTime()));return G}function w(u){if(u&&u.hasAttribute("data-timestamp"))return Number(u.getAttribute("data-timestamp"))}
...
(function(){function a(c,e,f,g){return{template:function(h,l){var m=l.mdHideIcons;l=l.ariaLabel||l.mdPlaceholder;var v="all"===m||"calendar"===m?"":'<md-button class="md-datepicker-button md-icon-button" type="button" tabindex="-1" aria-hidden="true" ng-click="ctrl.openCalendarPane($event)"><md-icon class="md-datepicker-calendar-icon" aria-label="md-calendar" md-svg-src="'+c.mdCalendar+'"></md-icon></md-button>',n="";"all"!==m&&"triangle"!==m&&(n='<md-button type="button" md-no-ink class="md-datepicker-triangle-button md-icon-button" ng-click="ctrl.openCalendarPane($event)" aria-label="{{::ctrl.locale.msgOpenCalendar}}"><div class="md-datepicker-expand-triangle"></div></md-button>',
...
(this.inputMask.parent().prop("clientHeight")-this.ngInputElement.prop("clientHeight"))/2;e=f.top-g.top-this.topMargin;var h=f.left-g.left-this.leftMargin,l=0>g.top&&0==document.body.scrollTop?-g.top:document.body.scrollTop,m=0>g.left&&0==document.body.scrollLeft?-g.left:document.body.scrollLeft;g=l+this.$window.innerHeight;var v=m+this.$window.innerWidth;this.inputMask.css({position:"absolute",left:this.leftMargin+"px",top:this.topMargin+"px",width:f.width-1+"px",height:f.height-2+"px"});h+360>v&&
(0<v-360?h=v-360:(h=m,f=this.$window.innerWidth/360,c.style.transform="scale("+f+")"),c.classList.add("md-datepicker-pos-adjusted"));e+368>g&&g-368>l&&(e=g-368,c.classList.add("md-datepicker-pos-adjusted"));c.style.left=h+"px";c.style.top=e+"px";document.body.appendChild(c);this.$$rAF(function(){c.classList.add("md-pane-open")})};b.prototype.detachCalendarPane=function(){this.$element.removeClass("md-datepicker-open");this.mdInputContainer&&this.mdInputContainer.element.removeClass("md-datepicker-open");
...
-1)});return w}function p(w){e.getClosest(w.target,"md-fab-trigger")&&t.toggle();e.getClosest(w.target,"md-fab-actions")&&t.close()}var t=this,H=0;t.open=function(){b.$evalAsync("vm.isOpen = true")};t.close=function(){b.$evalAsync("vm.isOpen = false");d.find("md-fab-trigger")[0].focus()};t.toggle=function(){b.$evalAsync("vm.isOpen = !vm.isOpen")};t.$onInit=function(){t.direction=t.direction||"down";t.isOpen=t.isOpen||!1;t.currentActionIndex=-1;d.addClass("md-animations-waiting");h();m();v()};1===
...
"scale("+q+")",g.style.transitionDelay="0ms",m&&(m.style.transitionDelay=".3s"),angular.forEach(v,function(r,x){r.style.transitionDelay=25*(v.length-x)+"ms"})):(l.style.pointerEvents="none",g.style.transform="scale(1)",g.style.top="0",c.hasClass("md-right")&&(g.style.left="0",g.style.right=null),c.hasClass("md-left")&&(g.style.right="0",g.style.left=null),g.style.transitionDelay="200ms",m&&(m.style.transitionDelay="0ms"),angular.forEach(v,function(r,x){r.style.transitionDelay=200+25*x+"ms"}))}}}return{addClass:function(c,
...
function mdMaxlengthDirective(a,b){function d(c,e,f,g){function h(k){k=x&&!y&&angular.isString(k)?k.trim():k;if(void 0===k||null===k)k="";return String(k).length}function l(){q&&q.parent()&&q.text(h(e.val())+" / "+m)}var m=parseInt(f.mdMaxlength);isNaN(m)&&(m=-1);var v=g[0],n=g[1],q,r,x=angular.isDefined(f.ngTrim)?b.parseAttributeBoolean(f.ngTrim):!0,y="password"===f.type;c.$watch(f.mdMaxlength,function(k){m=k});v.$validators["md-maxlength"]=function(k,p){if(!angular.isNumber(m)||0>m)return!0;l();
k=e.val()||p;if(void 0===k||null===k)k="";k=x&&!y&&angular.isString(k)?k.trim():k;return String(k).length<=m};v.$isEmpty=function(k){return 0===h(k)};b.nextTick(function(){r=angular.element(n.element[0].querySelector(".md-errors-spacer"));q=angular.element('<div class="md-char-counter">');r.append(q);f.$observe("ngTrim",function(k){x=angular.isDefined(k)?b.parseAttributeBoolean(k):!0});c.$watch(f.mdMaxlength,function(k){angular.isNumber(k)&&0<k?(q.parent().length||a.enter(q,r),l()):a.leave(q)})})}
...
A.attr("md-position-mode")||A.attr("md-position-mode",B+" target");A=A.children().eq(0);q(A[0])||A.attr("ng-click","$mdMenu.open($event)");A.attr("aria-label")||A.attr("aria-label","Open List Menu")}function m(A){"div"==A?(k=angular.element('<div class="md-no-style md-list-item-inner">'),k.append(f.contents()),f.addClass("md-proxy-focus")):(k=angular.element('<div class="md-button md-no-style">   <div class="md-list-item-inner"></div></div>'),A=angular.element('<md-button class="md-no-style"></md-button>'),
...
p){if(n.isOpen){n.isOpen=!1;e.nextTick(function(){n.onIsOpenChanged(n.isOpen)});var t=angular.extend({},p,{skipFocus:k});c.$emit("$mdMenuClose",d,t);a.hide(null,p);k||(k=n.restoreFocusTo||d.find("button")[0],k instanceof angular.element&&(k=k[0]),k&&k.focus())}};this.positionMode=function(){var k=(b.mdPositionMode||"target").split(" ");1===k.length&&k.push(k[0]);return{left:k[0],top:k[1]}};this.offsets=function(){var k=(b.mdOffset||"0 0").split(" ").map(parseFloat);if(2===k.length)return{left:k[0],
...
MenuItemController.prototype.initClickListeners=function(){function a(m){if(m)h.off("click",l);else h.on("click",l)}var b=this,d=this.ngModel,c=this.$scope,e=this.$attrs,f=this.mode;this.handleClick=angular.bind(this,this.handleClick);var g=this.iconEl,h=angular.element(this.buttonEl),l=this.handleClick;e.$observe("disabled",a);a(e.disabled);d.$render=function(){b.clearNgAria();if("radio"==f){var m=e.ngValue?c.$eval(e.ngValue):e.value;m=d.$modelValue==m}else m=d.$modelValue;m?(g.style.display="",
...
function MenuItemDirective(a,b,d){return{controller:"MenuItemController",require:["mdMenuItem","?ngModel"],priority:b.BEFORE_NG_ARIA,compile:function(c,e){function f(n,q,r){r=r||c;r instanceof angular.element&&(r=r[0]);r.hasAttribute(n)||r.setAttribute(n,q)}function g(n){n=a.prefixer(n);angular.forEach(n,function(q){if(c[0].hasAttribute(q)){var r=c[0].getAttribute(q);m[0].setAttribute(q,r);c[0].removeAttribute(q)}})}var h=e.type;if("checkbox"!==h&&"radio"!==h||!c.hasClass("md-in-menu-bar"))f("role",
"menuitem",c[0].querySelector("md-button, button, a"));else{var l=c[0].textContent,m=angular.element('<md-button type="button"></md-button>'),v='<md-icon md-svg-src="'+d.mdChecked+'"></md-icon>';m.html(l);m.attr("tabindex","0");angular.isDefined(e.mdPreventMenuClose)&&m.attr("md-prevent-menu-close",e.mdPreventMenuClose);c.html("");c.append(angular.element(v));c.append(m);c.addClass("md-indent").removeClass("md-in-menu-bar");f("role","checkbox"===h?"menuitemcheckbox":"menuitemradio",m);g("ng-disabled")}return function(n,
...
function SelectMenuDirective(a,b,d,c){function e(g,h,l,m){function v(r){13!=r.keyCode&&32!=r.keyCode||n(r)}function n(r){var x=b.getClosest(r.target,"md-option"),y=x&&angular.element(x).data("$mdOptionController");if(x&&y){if(x.hasAttribute("disabled"))return r.stopImmediatePropagation(),!1;var k=q.hashGetter(y.value),p=angular.isDefined(q.selected[k]);g.$apply(function(){q.isMultiple?p?q.deselect(k):q.select(k,y.value):p||(q.deselect(Object.keys(q.selected)[0]),q.select(k,y.value));q.refreshViewValue()})}}
var q=m[0];h.addClass("_md");c(h);h.on("click",n);h.on("keypress",v)}function f(g,h,l){function m(){var p=n.ngModel.$modelValue||n.ngModel.$viewValue||[];if(angular.isArray(p)){var t=Object.keys(n.selected),H=p.map(n.hashGetter);t=t.filter(function(A){return-1===H.indexOf(A)});t.forEach(n.deselect);H.forEach(function(A,w){n.select(A,p[w])})}}function v(){var p=n.ngModel.$viewValue||n.ngModel.$modelValue;Object.keys(n.selected).forEach(n.deselect);n.select(n.hashGetter(p),p)}var n=this;n.isMultiple=
...
n.hashGetter=function(w,B){H.$value=w;return A(B||g,H)}}else n.hashGetter=function(w){return angular.isObject(w)?"object_"+(w.$$mdSelectId||(w.$$mdSelectId=++selectNextId)):w};n.setMultiple(n.isMultiple)};n.selectedLabels=function(p){p=p||{};p=p.mode||"html";var t=b.nodesToArray(l[0].querySelectorAll("md-option[selected]"));if(t.length){var H;"html"==p?H=function(A){if(A.hasAttribute("md-option-empty"))return"";var w=A.innerHTML,B=A.querySelector(".md-ripple-container");B&&(w=w.replace(B.outerHTML,
...
(x.id="select_option_"+b.nextUid());h.attr(x)}var q=m[0],r=m[1];d(h);r.isMultiple&&(h.addClass("md-checkbox-enabled"),h.prepend(CHECKBOX_SELECTION_INDICATOR.clone()));angular.isDefined(l.ngValue)?g.$watch(l.ngValue,v):angular.isDefined(l.value)?v(l.value):g.$watch(function(){return h.text().trim()},v);l.$observe("disabled",function(x){x?h.attr("tabindex","-1"):h.attr("tabindex","0")});g.$$postDigest(function(){l.$observe("selected",function(x){angular.isDefined(x)&&("string"==typeof x&&(x=!0),x?(r.isMultiple||
r.deselect(Object.keys(r.selected)[0]),r.select(q.hashKey,q.value)):r.deselect(q.hashKey),r.refreshViewValue())})});a.attach(g,h);n();g.$on("$destroy",function(){r.removeOption(q.hashKey,q)})}function f(g){this.selected=!1;this.setSelected=function(h){h&&!this.selected?g.attr({selected:"selected","aria-selected":"true"}):!h&&this.selected&&(g.removeAttr("selected"),g.attr("aria-selected","false"));this.selected=h}}f.$inject=["$element"];return{restrict:"E",require:["mdOption","^^mdSelectMenu"],controller:f,
...
var h={find:f,waitFor:g};return function(l,m){if(angular.isUndefined(l))return h;var v=!0===m,n=h.find(l,v);return!n&&v?h.waitFor(l):!n&&angular.isUndefined(m)?e(h,l):n}}function SidenavFocusDirective(){return{restrict:"A",require:"^mdSidenav",link:function(){}}}
...
{prev:null,current:null,next:null,items:[],add:l,refreshElements:m}}function f(h){function l(){200<+d.now()-v?(m=!1,h.triggerHandler("$scrollend")):(h.triggerHandler("$scroll"),b.throttle(l))}var m,v;h.on("scroll touchmove",function(){m||(m=!0,b.throttle(l),h.triggerHandler("$scrollstart"));h.triggerHandler("$scroll");v=+d.now()})}var g=d.checkStickySupport();return function(h,l,m){var v=l.controller("mdContent");if(v)if(g)l.css({position:g,top:0,"z-index":2});else{var n=v.$element.data("$$sticky");
...
function MdTabsTemplate(a,b){function d(c,e,f,g){function h(){c.$watch("connected",function(v){!1===v?g.enableDisconnect&&b.disconnectScope(m):l()});c.$on("$destroy",l)}function l(){g.enableDisconnect&&b.reconnectScope(m)}if(g){var m=g.enableDisconnect?c.compileScope.$new():c.compileScope;e.html(c.template);a(e.contents())(m);return b.nextTick(h)}}return{restrict:"A",link:d,scope:{template:"=mdTabsTemplate",connected:"=?mdConnectedIf",compileScope:"=mdScope"},require:"^?mdTabs"}}
...
function(){l.content=e});this.resolve=function(){f.hide("ok")}}}function c(f,g,h,l,m){function v(x,y,k){e=k.textContent||k.content;var p=!l("gt-sm");y=h.extractElementByName(y,"md-toast",!0);k.element=y;k.onSwipe=function(t){t=t.type.replace("$md.","");var H=t.replace("swipe","");"down"===H&&-1!==k.position.indexOf("top")&&!p||"up"===H&&(-1!==k.position.indexOf("bottom")||p)||("left"===H||"right"===H)&&p||(y.addClass("md-"+t),h.nextTick(g.cancel))};k.openClass=q(k.position);y.addClass(k.toastClass);
...
x.toast.actionKey&&m.off("keydown");y.off("$md.swipeleft $md.swiperight $md.swipeup $md.swipedown",k.onSwipe);k.parent&&k.parent.addClass("md-toast-animating");k.openClass&&k.parent.removeClass(k.openClass);return(!0===k.$destroy?y.remove():f.leave(y)).then(function(){k.parent&&k.parent.removeClass("md-toast-animating");h.hasComputedStyle(k.parent,"position","static")&&k.parent.css("position","")})}function q(x){return l("gt-xs")?"md-toast-open-"+(-1<x.indexOf("top")?"top":"bottom"):"md-toast-open-bottom"}
function r(x){var y=function(k){"Escape"===k.key&&g.hide(!1);x&&k.key===x&&k.ctrlKey&&g.hide("ok")};m.on("keydown",y)}return{onShow:v,onRemove:n,toastClass:"",position:"bottom left",themable:!0,hideDelay:3E3,autoWrap:!0,transformTemplate:function(x,y){if(y=y.autoWrap&&x&&!/md-toast-content/g.test(x)){y=document.createElement("md-template");y.innerHTML=x;for(x=0;x<y.children.length;x++)if("MD-TOAST"===y.children[x].nodeName){var k=angular.element('<div class="md-toast-content">');k.append(angular.element(y.children[x].childNodes));
...
function mdToolbarDirective(a,b,d,c,e){var f=angular.bind(null,d.supplant,"translate3d(0,{0}px,0)");return{template:"",restrict:"E",link:function(g,h,l){function m(){function v(u){var z=h.parent().find("md-content");!k&&z.length&&n(null,z);u=g.$eval(u);!1===u?p():p=r()}function n(u,z){z&&h.parent()[0]===z.parent()[0]&&(k&&k.off("scroll",w),k=z,p=r())}function q(u){u=u?u.target.scrollTop:H;B();t=Math.min(y/A,Math.max(0,t+u-H));h.css(b.CSS.TRANSFORM,f([-t*A]));k.css(b.CSS.TRANSFORM,f([(y-t)*A]));H=
...
H))}else v.visibleWatcher=v.$watch("mdVisible",H);v.$watch("mdDirection",y);n.one("$destroy",K);B.one("$destroy",K);v.$on("$destroy",function(){t(!1);C&&C.destroy();M&&M.disconnect();n.remove()});-1<n.text().indexOf(e.startSymbol())&&v.$watch(function(){return n.text().trim()},x)}function t(K){t.queued&&t.value===!!K||!t.queued&&v.mdVisible===!!K||(t.value=!!K,t.queued||(K?(t.queued=!0,O=a(function(){v.mdVisible=t.value;t.queued=!1;O=null;v.visibleWatcher||H(v.mdVisible)},v.mdDelay)):f.nextTick(function(){v.mdVisible=
